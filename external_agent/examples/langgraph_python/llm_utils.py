...
def log_tool_usage(event_type: str, details: dict):
    """Enhanced logging for tool usage"""
    if event_type == "tool_start":
        logger.info(f"🔧 Starting tool: {details.get('name')} with input: {details.get('input')}")
    elif event_type == "tool_end":
        logger.info(f"✅ Tool completed: {details.get('name')}")
        logger.info(f"Tool output: {details.get('output')}")
    elif event_type == "llm":
        logger.info(f"🤖 LLM response: {details.get('response')}")

async def get_llm_stream(messages: List[Message], model: str, thread_id: str, tools):
    logger.info("🚀 Starting new conversation with model: " + model)
    logger.info(f"Initial messages: {json.dumps([m.dict() for m in messages], indent=2)}")
    
    if tools:
        use_tools = True
    else:
        use_tools = False
    send_tool_events = True  # Changed to True to see tool events
        ...
        if 'gpt' in model:
            ...
            client_model_instance = APIClient(
                credentials=Credentials(
                    url=WATSONX_URL, 
                    token=get_access_token(WATSONX_API_KEY)
                ),
                space_id=WATSONX_SPACE_ID
            )
            model_instance = ChatWatsonx(model_id=model, watsonx_client=client_model_instance)

        graph = create_react_agent(model_instance, tools=tools if use_tools else [])
            ...
            if kind == "on_chat_model_stream":
                ...
                if content:
                    if isinstance(content, str):
                        log_tool_usage("llm", {"response": content})
                ...
                log_tool_usage("tool_start", {
                    "name": event['name'],
                    "input": event['data'].get('input')
                })
                if send_tool_events:
                    # Tool start event formatting here
                    current_timestamp = int(time.time())
                    step_details = {
                        "type": "tool_calls",
                        "tool_calls": [
                    ...
                    yield format_resp(struct)
                    
            elif kind == "on_tool_end":
                log_tool_usage("tool_end", {
                    "name": event.get('name'),
                    "output": event.get('data', {}).get('output', {})
                })
                if send_tool_events:
                    # Tool end event formatting here
                    current_timestamp = int(time.time())
                    content = ''
